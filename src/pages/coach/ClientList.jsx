...
export default function ClientList() {
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="bg-white rounded-xl border shadow-sm p-4 sm:p-5 mb-6 w-full sm:max-w-md">
        ...
        <form onSubmit={handleInvite} className="flex gap-2">
          <input type="email" placeholder="Email du client" value={email}
            ...
            className="min-w-0 flex-1 border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
          <button type="submit" disabled={inviting}
            className="shrink-0 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 disabled:opacity-50">
            {inviting ? '...' : 'Inviter'}
          </button>
        </form>
        {status && (
